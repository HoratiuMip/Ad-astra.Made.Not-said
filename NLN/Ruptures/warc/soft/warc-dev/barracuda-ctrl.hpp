...
class BARRACUDA_CONTROLLER : virtual public DEVICE, virtual public NLN::Dev::BarracudaCTRL {
public:
    NLN_DESCRIPTOR_STRUCT_NAME_OVERRIDE( WARC_DEV_STR"::BARRACUDA_CONTROLLER" );
...
public:
    virtual int set( warc::MAIN& main, NLN_COMMS_ECHO_RT_ARG ) override {
        ...
        this->NLN::Dev::BarracudaCTRL::bind();

        this->set_soft_params( &main.imm_earth() );

        status = this->NLN::Dev::BarracudaCTRL::connect( NLN::Dev::BARRACUDA_CTRL_FLAG_TRUST_INVOKER );
    ...
    virtual int engage( warc::MAIN& main, NLN_COMMS_ECHO_RT_ARG ) override {
        ...
        WARC_ASSERT_RT_THIS( this->NLN::Dev::BarracudaCTRL::connected(), "BarraCUDA-CTRL not connected.", -1, -1 );
        ...
        _imm_control_th = std::thread( [ imm, this ] () -> void { 
            while( this->DEVICE::_engaged.load( std::memory_order_relaxed ) ) {
                ...
                int status = this->NLN::Dev::BarracudaCTRL::trust_resolve_recv( &info );
