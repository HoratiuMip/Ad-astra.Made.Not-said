...
#define WARC_IMM_CTRL_INSERT_TOKENS_CLEAR ( 1 )
    ...
    std::vector< IXT::HVEC< _SINK > >    sinks    = {};
    std::vector< IXT::HVEC< _DRAIN > >   drains   = {}; 
    std::list< _TOKEN >                  tokens   = {}; /* TO BE REPLACED with either PMR std::list or IXT's BLOCK_DIFFUSER. */

    
    template< typename S >
    size_t push_sink( S&& snk ) {
        this->sinks.emplace_back( IXT::HVEC< S >::allocc( std::forward< S >( snk ) ) );
        return this->sinks.size() - 1;
    }

    template< typename D >
    size_t push_drain( D&& dr ) {
        this->drains.emplace_back( IXT::HVEC< D >::allocc( std::forward< D >( dr ) ) );
    ...
    struct _CRS_INFO {
        ...
        float         _x_last         = 0.0;
        float         _dx_last        = 0.0;
        IXT::Ticker   _x_cross_tick   = { IXT::ticker_lap_epoch_init_t{} };
    } crs_info;

    struct _CIN_INFO {
        int           trigger_count   = 0;
        IXT::Ticker   tick            = { IXT::ticker_lap_epoch_init_t{} };
    } cin_info;
