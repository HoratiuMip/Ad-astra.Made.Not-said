...
struct CmdArgs : Descriptor {
    NLN_DESCRIPTOR_STRUCT_NAME_OVERRIDE( "CmdArgs" );

    CmdArgs( int argc, char* argv[], NLN::DWORD* result, NLN_COMMS_ECHO_ARG ) {
...
NLN::DWORD cvt_path( 
        ...
        }
    {
        ...
        NLN::BYTE header[ sizeof( XtFdx ) + sizeof( NLN::BYTE ) + sizeof( NLN::DWORD ) ];

        *( XtFdx* )header = FDX_CLUST2;
        *( ( NLN::BYTE* )header + sizeof( XtFdx ) ) = ( 0x0 & CLUST2_FILE_FMT_ORG_MSK ) | ( mode & CLUST2_FILE_FMT_MODE_MSK );
        *( NLN::DWORD* )( ( NLN::BYTE* )header + sizeof( XtFdx ) + sizeof( NLN::BYTE ) ) = vrtxs.size() >> 1;
