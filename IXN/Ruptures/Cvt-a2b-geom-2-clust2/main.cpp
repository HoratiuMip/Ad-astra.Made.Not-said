...
struct CmdArgs : Descriptor {
    IXN_DESCRIPTOR_STRUCT_NAME_OVERRIDE( "CmdArgs" );

    CmdArgs( int argc, char* argv[], ixN::DWORD* result, IXN_COMMS_ECHO_ARG ) {
        ...
        *result = 1;
        ...
        if( ixN::UBYTE sit = ( src_dir.empty() << 1 ) | out_dir.empty(); sit != 0b00 ) {
...
ixN::DWORD cvt_path( 
    ...
    File::for_each_in_dir_matching( src_dir.c_str(), rel.c_str(), [ & ] ( std::string_view mch ) -> ixN::DWORD {
        ...
        }
    {
        ...
        ixN::BYTE header[ sizeof( XtFdx ) + sizeof( ixN::BYTE ) + sizeof( ixN::DWORD ) ];

        *( XtFdx* )header = FDX_CLUST2;
        *( ( ixN::BYTE* )header + sizeof( XtFdx ) ) = ( 0x0 & CLUST2_FILE_FMT_ORG_MSK ) | ( mode & CLUST2_FILE_FMT_MODE_MSK );
        *( ixN::DWORD* )( ( ixN::BYTE* )header + sizeof( XtFdx ) + sizeof( ixN::BYTE ) ) = vrtxs.size() >> 1;
