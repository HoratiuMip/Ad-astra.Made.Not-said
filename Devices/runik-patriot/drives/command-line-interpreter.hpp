...
#define _RP_CLI_NO_ROUTE return { -1, "Invalid route." };
#define _RP_CLI_OK return { 0, "Route OK." }
 ...
 */
    ...
    _RP_CLI_ROUTE_FUNC( _track_seq ) { _RP_CLI_ASSERT_TOK_COUNT_GE( 3, "track seq <seq_number> <power> <duration_ms>"  );
        ...
        switch( atoi( tokens[ 0 ].c_str() ) ) {
            case 1: _track_drive->seq_1( pwr, ms ); return { 0, "Track sequence complete." };
            case 2: _track_drive->seq_2( pwr, ms ); return { 0, "Track sequence complete." };
            case 3: _track_drive->seq_3( pwr, ms ); return { 0, "Track sequence complete." };
            case 4: _track_drive->seq_4( pwr, ms ); return { 0, "Track sequence complete." };
            case 5: _track_drive->seq_5( pwr, ms ); return { 0, "Track sequence complete." };
        }
