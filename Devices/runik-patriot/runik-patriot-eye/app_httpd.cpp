...
std::function< bool() >   _acquire_frame;
...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (true) {
    if( false == rp::_acquire_frame() ) { vTaskDelay( 1000 ); continue; }
...
void startCameraServer( std::function< bool() > acquire_frame ) {
    rp::_acquire_frame = acquire_frame;
