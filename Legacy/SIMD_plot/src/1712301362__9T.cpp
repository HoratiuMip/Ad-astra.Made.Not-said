...
public:
    struct Wav : public EchoInvoker {
        ...
        Wav( std::string_view path, _ENGINE_ECHO_DFD_ARG ) {
            ...
            ECHO_ASSERT_AND_THROW( file_stream != nullptr, "Buffer alloc to stream file." );
            ...
            ECHO_ASSERT_AND_THROW( stream != nullptr, "Buffer alloc for stream." );
...
class Mat3 {
public:
    using Array = std::array< double, 9 >;
...
public:
    ...
    static std::optional< std::pair< Vec2, Vec2 > > clip_CyrusBeck( const Clust2& clst, Vec2 p1, Vec2 p2 ) {
        for( size_t idx = 0; idx < clst.vrtx_count(); ++idx ) {
            Vec2 v1 = clst( idx );
            Vec2 v2 = clst( ( idx + 1 ) % clst.vrtx_count() );
