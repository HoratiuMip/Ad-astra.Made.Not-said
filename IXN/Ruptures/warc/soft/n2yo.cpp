...
int _N2YO::burn_api_key( const char* key, const char* process ) {
    _WARC_IXN_COMPONENT_DESCRIPTOR( WARC_N2YO_STR"::burn_api_key()" );
...
std::string _N2YO::extract_api_key( const char* process ) {
    _WARC_IXN_COMPONENT_DESCRIPTOR( WARC_N2YO_STR"::extract_api_key()" );
...
std::string _N2YO::tufilin_positions_request( 
    ...
    _WARC_IXN_COMPONENT_DESCRIPTOR( WARC_N2YO_STR"::tufilin_positions_request()" );
...
std::string _N2YO::send_get_positions(
    ...
    _WARC_IXN_COMPONENT_DESCRIPTOR( WARC_N2YO_STR"::send_get_positions()" );
...
POSITIONS _N2YO::json_2_positions( std::string_view json ) {
    _WARC_IXN_COMPONENT_DESCRIPTOR( WARC_N2YO_STR"::json_2_positions()" );

    WARC_ASSERT_RT( json.data() != nullptr, "Json is NULL.", -1, {} );

    WARC_ECHO_RT_INTEL << "Parsing json.";

    ixN::UPtr< char[] > buffer{ ( char* )malloc( json.size() + 1 ) };
...
POSITIONS _N2YO::quick_position_xchg(
    ...
    _WARC_IXN_COMPONENT_DESCRIPTOR( WARC_N2YO_STR"::quick_position_xchg()" );
